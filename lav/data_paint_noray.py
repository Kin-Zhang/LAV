...
class Coordinator:
    def __init__(self, args):
        wandb.init(project='lav_point_painting', entity="wor")
...
def main(args):
    ...
    workers = [PointPainter(args) for _ in range(args.num_workers)]
    coordinator = Coordinator(args)
    num_frames = coordinator.num_frames

    num_completed = 0
    print('The num of frames to work:', num_frames)
    for idx in tqdm(range(0, num_frames, args.num_workers)):
        idxes = range(idx,min(idx+args.num_workers, num_frames))

        jobs = [worker.step(idx) for idx, worker in zip(idxes, workers)]
        
        for idx, sems, lidar, lidar_painted in jobs:
            num_completed += 1
